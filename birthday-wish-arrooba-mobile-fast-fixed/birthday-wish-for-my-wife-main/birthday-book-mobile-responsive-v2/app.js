...
(function () {
  ...
  function updateLeftPage(idx) {
    ...
    // Show the real memory photo for this chapter, if one is set
    if (polaroidPhoto) {
      if (item.image) {
        polaroidPhoto.classList.remove('is-visible');
        const loader = new Image();
        loader.onload = () => {
          polaroidPhoto.src = item.image;
          polaroidPhoto.alt = item.polaroidCaption || 'Our memory';
          requestAnimationFrame(() => polaroidPhoto.classList.add('is-visible'));
        };
        loader.onerror = () => { polaroidPhoto.classList.remove('is-visible'); polaroidPhoto.removeAttribute('src'); };
        loader.src = item.image;
      } else {
        polaroidPhoto.classList.remove('is-visible');
        polaroidPhoto.removeAttribute('src');
      }
    }
  }
  ...
  function updateNavButtons() {
    ...
    if (currentPage >= total - 1) {
      ...
      if (btnNextText) btnNextText.textContent = 'Close Book';
      if (btnNextIcon) btnNextIcon.textContent = '📕';
      if (btnNext) btnNext.classList.add('nav-btn--close');
      show(closeCta);
    } else {
      if (btnNext) btnNext.disabled = false;
      if (btnNextText) btnNextText.textContent = 'Next Page';
      if (btnNextIcon) btnNextIcon.textContent = '→';
      if (btnNext) btnNext.classList.remove('nav-btn--close');
      hide(closeCta);
    }
  ...
  // Happy Birthday melody for Arooba (loops continuously while music is on)
  const MELODY = [
    {f:392.00,d:.45},{f:392.00,d:.25},{f:440.00,d:.7},{f:392.00,d:.7},{f:523.25,d:.7},{f:493.88,d:1.1},
    {f:392.00,d:.45},{f:392.00,d:.25},{f:440.00,d:.7},{f:392.00,d:.7},{f:587.33,d:.7},{f:523.25,d:1.1},
    {f:392.00,d:.45},{f:392.00,d:.25},{f:783.99,d:.7},{f:659.25,d:.7},{f:523.25,d:.7},{f:493.88,d:.7},{f:440.00,d:1.1},
    {f:698.46,d:.45},{f:698.46,d:.25},{f:659.25,d:.7},{f:523.25,d:.7},{f:587.33,d:.7},{f:523.25,d:1.2}
  ];
  function initAudio() {
    if (!audioCtx) {
  ...
  function nextMelNote() {
    ...
    playNote(n.f, n.d * 0.75, 0.035);
    melTimer = setTimeout(nextMelNote, (n.d + 0.25) * 1000);
  }
  ...
  // ─── 13. Customizer Modal ───────────────────────────────────────────
  const CONTENT_VERSION = '2'; // bump this whenever DEFAULT_REASONS changes, to force old saved edits to refresh

  function loadReasons() {
    try {
      const savedVersion = localStorage.getItem('ril_reasons_version');
      if (savedVersion !== CONTENT_VERSION) {
        localStorage.setItem('ril_reasons_version', CONTENT_VERSION);
        localStorage.removeItem('ril_reasons');
  ...
  function wireEvents() {
    ...
    if (btnPrev) btnPrev.addEventListener('click', () => { btnPrev.blur(); prevPage(); });
    if (btnNext) btnNext.addEventListener('click', () => { btnNext.blur(); nextPage(); });
    ...
    if (btnCustomizer) btnCustomizer.addEventListener('click', () => { btnCustomizer.blur(); openModal(); });
